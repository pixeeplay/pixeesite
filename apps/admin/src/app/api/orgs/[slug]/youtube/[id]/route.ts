...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const data: any = {};
  for (const k of ['title', 'description', 'thumbnail', 'channel', 'category', 'featured'] as const) if (b[k] !== undefined) data[k] = b[k];
  if (b.position !== undefined) data.position = Number(b.position);
  const item = await (db as any).youtubeVideo.update({ where: { id }, data });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  try { await requireOrgMember(slug, ['owner', 'admin', 'editor']); } catch (e: any) { return NextResponse.json({ error: e.message }, { status: 403 }); }
  const db = await getTenantPrisma(slug);
  await (db as any).youtubeVideo.delete({ where: { id } });
