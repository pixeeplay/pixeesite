...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const data: any = {};
  for (const k of ['authorName', 'authorTitle', 'authorAvatar', 'videoUrl', 'posterImage', 'quote', 'featured', 'published'] as const) {
    if (b[k] !== undefined) data[k] = b[k];
  }
  if (b.rating !== undefined) data.rating = b.rating ? Number(b.rating) : null;
  if (b.position !== undefined) data.position = Number(b.position);
  if (Array.isArray(b.tags)) data.tags = b.tags;
  const item = await (db as any).testimonial.update({ where: { id }, data });
